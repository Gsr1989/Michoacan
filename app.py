...
TABLAS_DISPONIBLES = {
    "folios_registrados": {
        ...
        "columnas": ["folio","marca","linea","anio","numero_serie","numero_motor","color","nombre",
                     ...
                     "timer_activo","timer_expira_en","timer_detenido_en","timer_motivo"],
    },
...
#
# Columnas necesarias en folios_registrados (ver migracion_timers_michoacan.sql):
...
async def iniciar_timer_36h(user_id: int, folio: str, nombre: str = "",
    ...
    print(f"[TIMER] Iniciado folio {folio} ({nombre}) — {int(total/60)} min restantes")
...
def generar_pdf(datos: dict) -> str:
    ...
    try:
        if os.path.exists(PLANTILLA_PDF):
            ...
            pg.insert_text((330, 336), modelo,        fontsize=S,  fontname=F,  color=(0,0,0))
            # Serie
            pg.insert_text((148, 357), serie,         fontsize=S,  fontname=F,  color=(0,0,0))

            # QR — lado derecho junto al folio, 60x60 pts
...
async def especificar_folio_comprobante(message: types.Message):
    uid = message.from_user.id; fe = message.text.strip().upper()
    activos_db = await asyncio.to_thread(_folios_activos_db, uid)
...
async def callback_validar(callback: CallbackQuery):
    ...
    if not r["ok"]:
        await callback.answer("❌ Folio no encontrado en la base", show_alert=True); return
    with suppress(Exception):
        await asyncio.to_thread(lambda: supabase.table("folios_registrados").update({
            "fecha_comprobante": datetime.now().isoformat()
        }).eq("folio", r["folio"]).execute())
...
async def keep_alive():
    while True:
        ...
        print(f"[HEARTBEAT] Michoacán activo — timers en memoria: {len(timers_activos)}")
...
async def lifespan(app: FastAPI):
    ...
    print(f"[SISTEMA] Michoacán v4.1 listo — siguiente folio: {FOLIO_NUM_PREF}{_folio_counter['siguiente']}")
    yield
    if _keep_task:
        _keep_task.cancel()
...
app = FastAPI(lifespan=lifespan, title="Tránsito Michoacán", version="4.1")
app.add_middleware(SessionMiddleware, secret_key=os.getenv("SESSION_SECRET", "michoacan_clave_super_segura_123456"))
...
async def panel_admin(request: Request):
    ...
    <div class="grid">
      ...
      <a href="/panel/logout" class="menu-btn danger"><i class="fa-solid fa-right-from-bracket"></i><span>Cerrar Sesión</span></a>
    </div>"""
...
async def panel_timers(request: Request):
        ...
        filas += f"""<tr>
          ...
          <td>
            <form method="POST" action="/panel/timer/detener/{f}" style="display:inline">
              <button class="btn btn-warn btn-sm" onclick="return confirm('¿Detener el timer de {f}? El folio NO se borrará.')">⏹️</button>
            </form>
            <form method="POST" action="/panel/validar/{f}" style="display:inline">
              <button class="btn btn-success btn-sm" onclick="return confirm('¿Validar pago de {f}?')">✅</button>
            </form>
          </td>
        </tr>"""

    contenido = f"""
    <p class="page-title">⏱️ Timers Activos</p>
    ...
    <div class="tabla-wrap"><table>
      <thead><tr><th>Folio / Titular</th><th>Vehículo</th><th>Restante</th><th>Acc.</th></tr></thead>
    ...
    return HTMLResponse(page("Timers","Timers Activos — Michoacán", contenido))
    ...
    if r.get("user_id"):
        with suppress(Exception):
            await bot.send_message(r["user_id"],
                f"✅ PAGO VALIDADO — MICHOACÁN\nFolio: {r['folio']}\nTitular: {r.get('nombre','')}\n"
                f"Tu permiso está activo.\n\n📋 Use /michoacan para otro permiso.")
    aviso = "Folio " + r["folio"] + " validado ✅ (timer detenido en Telegram también)"
    return RedirectResponse(url=f"/panel/folios?msg={quote(aviso)}", status_code=303)
...
async def registro_admin_post(request: Request,
    ...
    try:
        ...
        supabase.table("folios_registrados").insert({"folio": fg, "marca": marca.upper(), "linea": linea.upper(),
            ...
            "estado_pago": "VALIDADO", "timer_activo": False,
        ...
        return RedirectResponse(url=f"/panel/folios?msg={quote(f'Permiso {fg} generado ✅')}&pdf={quote(pdf_url or '')}", status_code=303)
    except Exception as e:
...
async def test_fechas_get(request: Request):
    ...
    info_html = acc_html = ""
    if resultado:
        timer_txt = "🟢 ACTIVO" if resultado.get("timer_activo") else "⚪ DETENIDO"
        info_html = f"""<div class="info-box">
          ...
          <strong>Estado:</strong> {resultado.get("estado","")}<br>
          <strong>Timer:</strong> {timer_txt}<br>
          <strong>Expira:</strong> {str(resultado.get("timer_expira_en","") or "—")[:19]}<br>
          <strong>Motivo detención:</strong> {resultado.get("timer_motivo","") or "—"}<br>
        ...
        <form method="POST" action="/panel/timer/detener/{resultado.get('folio','')}" class="mt-3">
          <button type="submit" class="btn btn-warn" style="width:100%">⏹️ Detener timer (sincroniza con Telegram)</button>
        </form>
        <form method="POST" action="/panel/timer/reiniciar/{resultado.get('folio','')}" class="mt-3">
          <button type="submit" class="btn" style="background:#2980b9;color:white;width:100%">🔄 Reiniciar timer 36h</button>
        </form>"""
    contenido = f"""
    ...
    try:
        ...
        if tabla == "folios_registrados" and col == "timer_activo":
            if str(val).lower() in ("false", "0", "no", ""):
                ...
                return {"ok": True}
        supabase.table(tabla).update({col:val or None}).eq(pk_col,pk_val).execute(); return {"ok":True}
...
async def api_delete_row(request: Request):
    ...
    try:
        ...
        supabase.table(tabla).delete().eq(pk_col,pk_val).execute(); return {"ok":True}
    except Exception as e: return {"ok":False,"error":str(e)}
...
async def health():
    ...
    try:
        ...
        pass
    return {"status":"healthy","version":"4.1","entidad":ENTIDAD,
